...
import { getPeriodRange, parseReportDate, type ReportPeriod } from "./dateRange";
...
export function asReportPeriod(v: unknown): ReportPeriod {
  if (v === "daily" || v === "weekly" || v === "monthly" || v === "yearly") return v;
  return "daily";
}

export type IncomeExpenseReportPayload = {
  period: ReportPeriod;
  start: string;
  endExclusive: string;
  income: number;
...
export async function computeIncomeExpenseReport(
  period: ReportPeriod,
  dateInput: unknown,
): Promise<IncomeExpenseReportPayload> {
